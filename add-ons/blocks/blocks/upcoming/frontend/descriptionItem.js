...
export default function descriptionItem(props) {
	...
	return (
		<a href={item.link} className={"description__item " + textAlignment}>
			{ showImages && 
				<img className="description__image" src={item.image?.sizes?.large?.url} />	
			}
			{ !showImages && 
			...
			<div className='description__text'>
				...
				<div class="description__data">{formatDateRange(item.start, item.end)}<br/>
					<div class="description__subtitle">{ showAudience && item.audience && <span>{item.audience}</span>}{ showSpeaker == 'name' && item.speaker && <span>{item.speaker.name}</span>}{ showLocation && item.location && <span className=''>{location}</span>}</div>
				</div>
			</div>
		</a>
	)
