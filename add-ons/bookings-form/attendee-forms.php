...
class EM_Attendees_Form {
	...
	public static function get_form_template(){
	    if( empty(self::$form_template )){
    		self::$form_template = apply_filters('em_attendees_form_get_form_template', array (
				'attendee_intro' => array ( 'label' => __('Title','events-manager'), 'type' => 'html', 'fieldid'=>'attendee_intro', 'options_html_content'=>'<strong>'.sprintf(__('Attendee %s','em-pro'), '#NUM#'). '</strong>'),
	...
	public static function get_fields($event) {
		global $EM_Event;
		$fieldset = [];
		$fields = self::get_form($event)->form_fields;
	...
	public static function tickets_form($EM_Ticket){
		...
		$col_numbers = $EM_Ticket->get_event()->get_bookings()->get_tickets()->get_ticket_columns();
		...
		?>
		<tr class="em-attendee-details" id="em-attendee-details-<?php echo $EM_Ticket->ticket_id; ?>" <?php if( $min_spaces == 0 ) echo ''?>>
	 ...
	 * For rendering, alpine.js is needed and an appropriate x-data must be given.
	 * 
	 * 
	 * @param EM_Ticket $EM_Ticket
	 */
	public static function ticket_form(EM_Ticket $EM_Ticket) : void {
		...
		if( self::$form_id > 0 ){
			...
			if( $EM_Ticket->is_available() ) {
				...
				<div class="em-attendee-fields-template">
					<template 
						x-for="ticket in Array.apply(null, {length: ticketCount<?php echo $EM_Ticket->ticket_id ?>}).map(Number.call, Number)" 
						:key="ticket"><div class="p-4 bg-gray-100 first:rounded-tl-md last:rounded-br-md mt-4 mb-2"><h4><?php _e("Attendee", "em-pro"); ?> <span x-text="ticket + 1"></span></h4><?php echo $form; ?><div>
					</template>
				</div>
				<?php
			}
	...
	public static function event_attendee_custom_form(){
		...
		?>
		<tr>
		<th><?php _e('Selected Attendee Form','em-pro'); ?></th>
		<td><select name="custom_attendee_form">
			<option value="0" <?php if( empty($event_form_id) ) echo 'selected="selected"'; ?>>[ <?php _e('Default','em-pro'); ?> ]</option>
			<option value="none" <?php if( 'none' === $event_form_id ) echo 'selected="selected"'; ?>>[ <?php esc_html_e('None','events-manager'); ?> ]</option>
