...
class EM_Coupons_Admin {
	...
	static function admin_meta_box($EM_Event){
		...
		$coupon_args = array('owner'=>$owner, 'sitewide'=>1, 'eventwide'=>1 );
		$coupons = apply_filters('em_coupons_admin_meta_box_coupons', EM_Coupons::get($coupon_args), $coupon_args, $EM_Event );
        if( count($coupons) > 0 ): ?>
			...
			<?php foreach($coupons as $EM_Coupon): /* @var $EM_Coupon EM_Coupon */ ?>
				<p style="margin:0px 0px 5px 0px">
					<?php if( apply_filters('em_coupons_admin_meta_box_show_code', true, $EM_Coupon, $EM_Event) ): ?>
						<strong><?php echo esc_html($EM_Coupon->coupon_code); ?></strong> -
					<?php endif; ?>
					<?php echo esc_html($EM_Coupon->get_discount_text()); ?><br />
			...
			<form id='coupons-filter' method='post' action=''>
				...
				<div class="tablenav">			
					<div class="alignleft actions">
						<div class="subsubsub">
							<a href='<?php echo em_add_get_params($_SERVER['REQUEST_URI'], array('view'=>null, 'pno'=>null)); ?>' <?php echo ( empty($_REQUEST['view']) ) ? 'class="current"':''; ?>><?php echo sprintf( esc_html( 'My %s', 'events-manager'), __('Coupons','em-pro')); ?> <span class="count">(<?php echo $coupons_mine_count; ?>)</span></a>
							<?php if( current_user_can('manage_others_bookings') ): ?>
							&nbsp;|&nbsp;
							<a href='<?php echo em_add_get_params($_SERVER['REQUEST_URI'], array('view'=>'others', 'pno'=>null)); ?>' <?php echo ( !empty($_REQUEST['view']) && $_REQUEST['view'] == 'others' ) ? 'class="current"':''; ?>><?php echo sprintf( esc_html( 'All %s', 'events-manager'), __('Coupons','em-pro')); ?> <span class="count">(<?php echo $coupons_all_count; ?>)</span></a>
							<?php endif; ?>
						</div>
					</div>
					<?php
						...
						foreach($EM_Bookings as $EM_Booking){ /* @var EM_Booking $EM_Booking */
							...
							<tr>
								<td>
									...
									<?php if( in_array($EM_Booking->booking_status, array(2,3)) ) echo '<em>('. $EM_Booking->get_status() . ')</em> '; ?>
								</td>
		...
		<div class='wrap nosubsub'>
			...
			<form id='coupon-form' method='post' action=''>
				...
				<table class="form-table">
					...
					<tr valign="top">
						<th scope="row"><?php esc_html_e('Name', 'events-manager') ?></th>
							<td><input type="text" name="coupon_name" value="<?php echo esc_attr($EM_Coupon->coupon_name); ?>" />
							<br />
						</td>
					</tr>
					<tr valign="top">
						<th scope="row"><?php esc_html_e('Description', 'events-manager') ?></th>
