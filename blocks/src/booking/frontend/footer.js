		...
		<div className="section">
			<div className="container button-group button-group--right">
				...
				{ wizzard.step > (wizzard.steps.tickets.enabled ? 0 : 1) && wizzard.step < 3 && 
					<button className="button button--secondary" onClick={() => {dispatch({type: "DECREMENT_WIZZARD"})}}>{__('Back', 'events')}</button> }
				...
				{ SUCCESS && <button className="button button--success" onClick={() => {dispatch({type: "RESET"})}}>{__("Close", "events")}</button> }
			</div>
		</div>
  )
}
