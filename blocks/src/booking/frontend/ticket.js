        ...
        <div className="card card--no-image bg-white my-8 card--shadow">
            ...
            { attendee_fields.map((field, key) => {
                ...
                <InputField
                    ...
                    onChange={(value) => dispatch({type: "SET_FIELD", payload: {form: 'ticket', index, field: field.fieldid, value: value}})}
                />)}
            ) }
          	</div>
            <div className="card__footer mt-8">
                ...
                <div className="card__supplemental">
					<button href="" className="button button--error button--icon button--pop" onClick={() => dispatch({type: "REMOVE_TICKET", payload: {index}})} disabled={ticket.min >= index + 1}><i class="material-icons">delete</i></button>
                </div>
            </div>
            </div>
