...
import { formatCurrency, fullPrice, ticketPrice } from './modules/priceUtils';
		...
		<div className="list ">
		  { Object.keys(data.available_tickets).map((id, key) =>
			  <div className="list__item" key={key}>
				  <div className="list__content">
					  ...
					  <div className="list__subtitle">{__("Base price:", "events")} {formatCurrency(data.available_tickets[id].price)}</div>
				  </div>
				  
				  <div className="list__actions">
					  <span className="button button--pseudo nowrap">{formatCurrency(ticketPrice(id))}</span>
					  { data.attendee_fields.length == 0 && <div className="number-picker">
						<button className="button button--primary button--icon" onClick={() => dispatch({type: "REMOVE_TICKET", payload: {id}})} disabled={data.available_tickets[id].min == countTicketsById(id)}></button>
						<input value={countTicketsById(data.available_tickets[id].id)}/>
						<button className="button button--primary button--icon" onClick={() => dispatch({type: "ADD_TICKET", payload: id})} disabled={data.available_tickets[id].max == countTicketsById(id)}></button>
					  </div> }
					  { data.attendee_fields.length > 0 && wizzard.step == 0 && <>
                        <button className="button button--primary button--icon" onClick={() => dispatch({type: "ADD_TICKET", payload: id})}><i className="material-icons">add_circle</i></button>
						</> }
				  </div> 
			  </div>
		  )}
		  { response.coupon.success &&
			  <div className="list__item" >
				  <div className="list__content">
					  <div className="list__title">{response.coupon.description || __("Coupon", "events")}</div>    
					  
				  </div>
				  <div className="list__actions">
					  <b className="button button--pseudo nowrap">{response.coupon.percent ? response.coupon.discount + " %" : formatCurrency(response.coupon.discount)}</b>
					  { data.attendee_fields.length == 0 && <div className="number-picker invisible">
