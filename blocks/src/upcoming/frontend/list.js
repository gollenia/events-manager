...
function list(props) {
	...
	return (
		<div className={"card card--image-left has-white-background card--shadow card--primary card--" + textAlignment}>
			{ showImages && <a href={item.link} className="card__image">
				<img src={item.image?.sizes?.large?.url} />	
				{ showSpeaker == 'image' && item.speaker && <span className='card__label card__label--image'><img src={item.speaker.image?.sizes?.thumbnail?.url}/>{item.speaker.name}</span>}
			</a> }
			<div className='card__content'>
				...
				{ (showAudience || showSpeaker || showLocation) && 
					<div class="card__footer card__subtitle card__pills">
						...
						{ showSpeaker == 'name' && item.speaker && <span className='card__pill event__audience'>{item.speaker.name}</span>}
