...
(() => {
/*!***********************************!*\
  !*** ./classes/events-manager.js ***!
  \***********************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _events_manager_css__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./events_manager.css */ "./classes/events_manager.css");
...
jQuery(document).ready(function ($) {
  ...
  if ($("#em-tickets-form").length > 0) {
    ...
    $("#em-tickets-add").on('click', function (e) {
      e.preventDefault();
      reset_ticket_forms(); //create copy of template slot, insert so ready for population

      let tickets = $('#em-tickets-form table tbody');
      let rowNo = tickets.length + 1;
      let slot = tickets.first('.em-ticket-template').clone(true).attr('id', 'em-ticket-' + rowNo).removeClass('em-ticket-template').addClass('em-ticket').appendTo($('#em-tickets-form table')); //change the index of the form element names
      ...
      slot.show().find('.ticket-actions-edit').trigger('click'); //refresh datepicker and values
  ...
  if ($('#em-bookings-table').length > 0) {
    ...
    $(document).on('click', '#em-bookings-table .tablenav-pages a', function () {
      let el = $(this);
      let form = el.parents('#em-bookings-table form.bookings-filter'); //get page no from url, change page, submit form
    ...
    let em_bookings_settings_dialog = {
      ...
      buttons: [{
        ...
        click: function (e) {
          e.preventDefault(); //we know we'll deal with cols, so wipe hidden value from main
    ...
    if ($("#em-bookings-table-settings").length > 0) {
      ...
      $('#em-bookings-table-export-form input[name=show_tickets]').on('click', export_overlay_show_tickets); //Sortables
    ...
    $(document).on('submit', '#em-bookings-table form.bookings-filter', function (e) {
      let el = $(this); //append loading spinner

      el.parents('#em-bookings-table').find('.table-wrap').first().append('<div id="em-loading" />'); //ajax call

      $.post(EM.ajaxurl, el.serializeArray(), function (data) {
        let root = el.parents('#em-bookings-table').first();
        root.replaceWith(data); //recreate overlays
  ...
  if (load_ui_css) em_load_jquery_css(); //previously in em-admin.php
  ...
  $('input#event-recurrence').on('change', updateShowHideRecurrence); // recurrency elements   
  ...
  }).trigger('change'); //Finally, add autocomplete here
  //Autocomplete
  ...
  if (window.moment) {
    ...
    $('.em-date-momentjs').each(function () {
      ...
      if (el.data('date-start') !== el.data('date-end')) {
        ...
        let day_end_string = replace_specials(day_start, day_end.format(el.data('date-format'))); // Output
...
function em_custom_mails() {
  ...
  if (trigger_selector.length == 1) {
    ...
    (_document$querySelect = document.querySelector('.em-nav-side')) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.classList.add('hidden');
    (_document$querySelect2 = document.querySelector('.em-nav-content')) === null || _document$querySelect2 === void 0 ? void 0 : _document$querySelect2.classList.remove('hidden');
    return;
  ...
  trigger_selector.forEach(element => {
    element.addEventListener("click", event => {
      ...
      (_document$querySelect3 = document.querySelectorAll('.em-nav-content')) === null || _document$querySelect3 === void 0 ? void 0 : _document$querySelect3.forEach(e => {
        e.classList.add('hidden');
      });
      event.target.classList.add('active');
      (_document$querySelect4 = document.querySelector(element.getAttribute('rel'))) === null || _document$querySelect4 === void 0 ? void 0 : _document$querySelect4.classList.remove('hidden');
    });
  });
  let accordion_items = document.querySelectorAll('.em-accordion-item');
  accordion_items.forEach(element => {
    element.addEventListener("click", event => {
      var _document$querySelect5, _document$getElementB;

      accordion_items.forEach(item => {
        item.classList.remove('active');
      });
      (_document$querySelect5 = document.querySelectorAll('.em-accordion-content')) === null || _document$querySelect5 === void 0 ? void 0 : _document$querySelect5.forEach(e => {
        e.classList.add('hidden');
      });
      event.target.classList.add('active');
      (_document$getElementB = document.getElementById(element.getAttribute('rel'))) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.classList.remove('hidden');
    });
  });
  ...
  mail_subgroups.forEach(element => {
    ...
    selector.addEventListener("change", event => {
      ...
      indicator === null || indicator === void 0 ? void 0 : indicator.setAttribute('data-status', event.target.value);
  ...
  (_document$getElementB2 = document.getElementById('booking-modal-close')) === null || _document$getElementB2 === void 0 ? void 0 : _document$getElementB2.addEventListener('click', event => {
    let element = document.getElementById('booking-modal');
    element.style.display = "none";
  });
  (_document$getElementB3 = document.getElementById('email-modal-close')) === null || _document$getElementB3 === void 0 ? void 0 : _document$getElementB3.addEventListener('click', event => {
