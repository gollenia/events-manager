...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click change', '.em-toggle', function (e) {
    e.preventDefault(); //show or hide advanced tickets, hidden by default
  ...
  if ($('.em-search-ajax').length > 0) {
    $(document).on('click', '.em-search-ajax a.page-numbers', function (e) {
      ...
      var vars = qvars[1]; //add data-em-ajax att if it exists
  ...
  if ($("#em-tickets-form").length > 0) {
    ...
    $("#em-tickets-add").on('click', function (e) {
      ...
      var slot = tickets.first('.em-ticket-template').clone(true).attr('id', 'em-ticket-' + rowNo).removeClass('em-ticket-template').addClass('em-ticket').appendTo($('#em-tickets-form table')); //change the index of the form element names
  ...
  if ($('#em-bookings-table').length > 0) {
    ...
    $(document).on('click', '#em-bookings-table .tablenav-pages a', function () {
      var el = $(this);
      var form = el.parents('#em-bookings-table form.bookings-filter'); //get page no from url, change page, submit form
    ...
    $(document).on('submit', '#em-bookings-table form.bookings-filter', function (e) {
      var el = $(this); //append loading spinner
  ...
  if (window.moment) {
    ...
    $('.em-date-momentjs').each(function () {
      ...
      if (el.data('date-start') !== el.data('date-end')) {
        ...
        var day_end_string = replace_specials(day_start, day_end.format(el.data('date-format'))); // Output
...
function em_setup_datepicker(wrap) {
  wrap = jQuery(wrap); //default picker vals
  //apply datepickers
  ...
  if (dateDivs.length > 0) {
    ...
    dateDivs.find('input.em-date-input-loc').each(function (i, dateInput) {
      ...
      var dateValue_value = dateValue.val(); //now set the value
...
function em_setup_timepicker(wrap) {
  wrap = jQuery(wrap); // Keep the duration between the two inputs.
  ...
  wrap.find(".event-form-when .em-time-range input.em-time-end").on('change', function () {
    ...
    if (start.val()) {
      if (start.em_timepicker('getTime') > end.em_timepicker('getTime') && (end_date.length == 0 || start_date == end_date)) {
        end.addClass("error");
      } else {
