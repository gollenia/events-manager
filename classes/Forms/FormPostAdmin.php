...
class Admin {
	...
	function manage_posts_columns($columns) {
		unset( $columns['date'] );
		$columns['type'] = esc_html__( 'Type', 'events' );
		return $columns;
	}

	public static function option_page() {
		echo "<div class='wrap'>";
		echo "<h1 class='wp-heading-inline' style='margin-bottom: 1rem'>" . esc_html__( 'Booking Forms', 'events' ) . "</h1>";
		...
		<table class="wp-list-table widefat fixed striped table-view-list posts">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Form Name', 'events' ); ?></th>
					<th><?php esc_html_e( 'Description', 'events' ); ?></th>
					<th><?php esc_html_e( 'Date', 'events' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
					...
					while ( $the_query->have_posts() ) {
						...
						<tr>
							...
							<div class="row-actions">
								<span class="edit"><a href="<?php echo esc_url( admin_url( 'post.php?post=' . get_the_ID() . '&action=edit' ) ); ?>" aria-label="bearbeiten"><?php esc_html_e( 'Edit', 'events' ); ?></a> | </span>
								<span class="trash"><a class="submitdelete" href="<?php echo add_query_arg( ['action' => 'trash', 'post' => get_the_ID(), '_wpnonce'=>wp_create_nonce('trash-post_'.get_the_ID()) ], admin_url( 'post.php' ) ); ?>" aria-label="bearbeiten"><?php esc_html_e( 'Delete', 'events' ); ?></a></span>
							</div>
							</td>
		...
		<?php
		echo "<h1 class='wp-heading-inline' style='margin-top: 2rem; margin-bottom: 1rem'>" . esc_html__( 'Attendee Forms', 'events' ) . "</h1>";
