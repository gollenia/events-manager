...
<div class="em-ticket-form">
	...
	<div class="em-ticket-form-main">
		<div class="ticket-name">
			<label title="<?php esc_attr_e('Enter a ticket name.','events'); ?>"><?php esc_html_e('Name','events') ?></label>
		...
		<div class="ticket-description">
			<label><?php esc_html_e('Description','events') ?></label>
		...
		<div class="ticket-price"><label><?php esc_html_e('Price','events') ?></label><input type="text" name="em_tickets[<?php echo $col_count; ?>][ticket_price]" class="ticket_price" value="<?php echo esc_attr($ticket->get_price_precise(true)) ?>" /></div>
		
		<div class="ticket-spaces">
			<label title="<?php esc_attr_e('Enter a maximum number of spaces (required).','events'); ?>"><?php esc_html_e('Spaces','events') ?></label>
	...
	<div class="em-ticket-form-advanced">
		<div class="ticket-spaces ticket-spaces-min">
			<label title="<?php esc_attr_e('Leave either blank for no upper/lower limit.','events'); ?>"><?php echo esc_html_x('At least','spaces per booking','events');?></label>
			<input type="text" name="em_tickets[<?php echo $col_count; ?>][ticket_min]" value="<?php echo esc_attr($ticket->ticket_min) ?>" class="ticket_min" />
			<?php esc_html_e('spaces per booking', 'events')?>
		</div>
		<div class="ticket-spaces ticket-spaces-max">
			<label title="<?php esc_attr_e('Leave either blank for no upper/lower limit.','events'); ?>"><?php echo esc_html_x('At most','spaces per booking', 'events'); ?></label>
		...
		<div class="ticket-dates em-time-range em-date-range">
			<div class="ticket-dates-from">
				<label title="<?php esc_attr_e('Add a start or end date (or both) to impose time constraints on ticket availability. Leave either blank for no upper/lower limit.','events'); ?>">
					<?php esc_html_e('Available from','events') ?>
				</label>
				...
				<div class="ticket-dates-from-recurring">
					...
					<?php esc_html_e('day(s)','events'); ?>
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_start_recurring_when]" class="ticket-dates-from-recurring-when">
						<option value="before" <?php if( isset($ticket->ticket_meta['recurrences']['start_days']) && $ticket->ticket_meta['recurrences']['start_days'] <= 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','events'),__('Before','events'))); ?></option>
						<option value="after" <?php if( !empty($ticket->ticket_meta['recurrences']['start_days']) && $ticket->ticket_meta['recurrences']['start_days'] > 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','events'),__('After','events'))); ?></option>
					</select>
				</div>
				<?php echo esc_html_x('at', 'time','events'); ?>
			...
			<div class="ticket-dates-to">
				<label title="<?php esc_attr_e('Add a start or end date (or both) to impose time constraints on ticket availability. Leave either blank for no upper/lower limit.','events'); ?>">
					<?php esc_html_e('Available until','events') ?>
				</label>
				...
				<div class="ticket-dates-to-recurring">
					...
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_end_recurring_when]" class="ticket-dates-to-recurring-when">
						<option value="before" <?php if( isset($ticket->ticket_meta['recurrences']['end_days']) && $ticket->ticket_meta['recurrences']['end_days'] <= 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','events'),__('Before','events'))); ?></option>
						<option value="after" <?php if( !empty($ticket->ticket_meta['recurrences']['end_days']) && $ticket->ticket_meta['recurrences']['end_days'] > 0) echo 'selected="selected"'; ?>><?php echo esc_html(sprintf(_x('%s the event starts','before or after','events'),__('After','events'))); ?></option>
					</select>
				</div>
				<?php echo esc_html_x('at', 'time','events'); ?>
		...
		<div class="ticket-required">
			<label title="<?php esc_attr_e('If checked every booking must select one or the minimum number of this ticket.','events'); ?>"><?php esc_html_e('Required?','events') ?></label>
		...
		<div class="ticket-type">
			<label><?php esc_html_e('Available for','events') ?></label>
			<select name="em_tickets[<?php echo $col_count; ?>][ticket_type]" class="ticket_type">
				<option value=""><?php _e('Everyone','events'); ?></option>
				<option value="members" <?php if($ticket->ticket_members) echo 'selected="selected"'; ?>><?php esc_html_e('Logged In Users','events'); ?></option>
				<option value="guests" <?php if($ticket->ticket_guests) echo 'selected="selected"'; ?>><?php esc_html_e('Guest Users','events'); ?></option>
			</select>
		</div>
		<div class="ticket-roles" <?php if( !$ticket->ticket_members ): ?>style="display:none;"<?php endif; ?>>
			<label><?php _e('Restrict to','events'); ?></label>
			<div>
				<?php 
