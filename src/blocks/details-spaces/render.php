		...
		<div class="event-details-data">
			<?php if ($attributes['showNumber'] && $spaces > 0) : ?>
				<?php if ($spaces <= $attributes['warningThreshold']) : ?>
					<div class="event-details-number"><?php echo sprintf(_n("Only %s space left", "Only %s spaces left", $spaces, "events-manager"), $spaces) ?></div>
				<?php else : ?>
					<div class="event-details-number"><?php echo sprintf(_n("%s space left", "%s spaces left", $spaces, "events-manager"), $spaces) ?></div>
				<?php endif; ?>
			<?php else : ?>
				<?php if ($spaces == 0) : ?>
					<div class="event-details-number"><?php echo __("No spaces left", "events-manager") ?></div>
				<?php elseif ($spaces <= $attributes['warningThreshold']) : ?>
					<div class="event-details-number"><?php echo __("Only few spaces left", "events-manager") ?></div>
				<?php else : ?>
					<div class="event-details-number"><?php echo __("Plenty of spaces left", "events-manager") ?></div>
				<?php endif; ?>
			<?php endif; ?>
		</div> 
	</div>
