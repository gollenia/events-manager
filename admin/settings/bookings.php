...
<div class="em-menu-bookings em-menu-group"  <?php if( !defined('EM_SETTINGS_TABS') || !EM_SETTINGS_TABS) : ?>style="display:none;"<?php endif; ?>>	
	...
	<h2 class="title"><?php echo sprintf(__( '%s Options', 'events'),__('General','events')); ?></h2>
	...
	<h2 class="title"><?php echo sprintf(__( '%s Options', 'events'),__('Pricing','events')); ?> </h2>
	...
	<h2 class="title"><?php _e( 'Customize Feedback Messages', 'events'); ?></h2>

	<table class='form-table' role="presentation">
		<?php
		Options::input ( __( 'Successful booking', 'events'), 'dbem_booking_feedback', __( 'When a booking is registered and confirmed.', 'events') );
		Options::input ( __( 'Successful pending booking', 'events'), 'dbem_booking_feedback_pending', __( 'When a booking is registered but pending.', 'events') );
		Options::input ( __( 'Not enough spaces', 'events'), 'dbem_booking_feedback_full', __( 'When a booking cannot be made due to lack of spaces.', 'events') );
		Options::textarea(__('Privacy Message', 'events'), 'dbem_privacy_message', __('You can type any HTML Content here.', 'events'));
		Options::textarea(__('Donation Message', 'events'), 'dbem_donation_message', __('A message that is displayed when a customer is asked to donate for other customers', 'events'));
		
		?>
	</table>


	<h2 class="title"><?php echo sprintf(__( '%s Options', 'events'),__('Ticket','events')); ?> </h2>
	
	<table class='form-table' role="presentation">
		<?php
		
		Options::checkbox ( __( 'Enable custom ticket ordering?', 'events'), 'dbem_bookings_tickets_ordering', __( 'When enabled, users can custom-order their tickets using drag and drop. If enabled, saved ordering supercedes the default ticket ordering below.', 'events') );
		...
		Options::select ( __( 'Order Tickets By', 'events'), 'dbem_bookings_tickets_orderby', $ticket_orders, __( 'Choose which order your tickets appear.', 'events') );
